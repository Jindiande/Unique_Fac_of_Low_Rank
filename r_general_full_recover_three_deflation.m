...
for j=1:length(r)
    ...
    for time2=1:repeat_time1
        D=null(randn(p,p-r(j))');
        X=random_ini_X(r(j),n,theta);
        Y=D*X;
        error_D_L4_sum2=0;
        error_D_ADMM_sum2=0;
        error_D_SPCA_sum2=0;
...
for i=1:size(error_average_r,1)
    a=reshape(error_average_r(i,1:size(error_average_r,2)),1,size(error_average_r,2))./(r.^(1/2));
    pj=plot(r(1:size(error_average_r,2)),a, Markers{i});
    h=[h,pj];
    hold on;
end
%}
