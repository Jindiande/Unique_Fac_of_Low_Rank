...
for i=1:size(error_average_r_theta,1)
    a=reshape(error_average_r_theta(i,1:size(error_average_r_theta,2)),1,size(error_average_r_theta,2))./(r.^(1/2));
    pj=plot(r(1:size(error_average_r_theta,2)),a, Markers{i});
    h=[h,pj];
    hold on;
end
%}
